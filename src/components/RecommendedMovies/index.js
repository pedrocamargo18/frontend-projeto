...
const RecommendedMovies = ({ title = "Filmes Recomendados", movies = [], loading = false, imagePath = "" }) => {
  const handleScroll = (direction) => {
    const scrollAmount = document.querySelector('.movie-rec-list')?.clientWidth || 0;
    const currentPosition = document.querySelector('.movie-rec-list')?.scrollLeft || 0;
    const newPosition = direction === 'left' ? currentPosition - scrollAmount : currentPosition + scrollAmount;
    document.querySelector('.movie-rec-list')?.scrollTo({ left: newPosition, behavior: 'smooth' });
  };
  ...
  return (
    <RecommendedSection>
      ...
      <MovieListContainer>
        <MovieList className="movie-rec-list">
          {loading ? (
            <p>Carregando...</p>
          ) : movies.length > 0 ? (
            movies.map((movie) => (
